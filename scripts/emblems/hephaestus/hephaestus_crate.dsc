...
hephaestus_key:
    ...
    display name: <&e>Hephaestus Key<&r>
    enchantments:
    - mending:1
    mechanisms:
        hides: ENCHANTS
    lore:
    - <&7>An iron key blessed by the
    - <&7>god of the forge.
    - <empty>
...
hephaestus_key_usage:
    ...
    events:
        ...
        - take item:hephaestus_key quantity:1

        # Start crate animation with pre-rolled results
        - run hephaestus_crate_animation def.tier:<[tier]> def.tier_color:<[tier_color]> def.loot:<[loot]> id:hephaestus_crate_<player.uuid>

        # Track statistics
        - flag player hephaestus.crates_opened:++
        - flag player hephaestus.tier.<[tier].to_lowercase>:++

        # Pity counter tracking (only while wearing divine armor)
        - if <proc[is_wearing_divine_armor].context[<player>|hephaestus]>:
            - if <[tier]> == OLYMPIAN:
                - flag player hephaestus.pity_counter:0
            - else:
                - flag player hephaestus.pity_counter:++

hephaestus_crate_animation:
    type: task
    debug: false
    definitions: tier|tier_color|loot
    script:
    # Store loot data as flag for early close detection
    - flag player hephaestus.crate.pending_loot:<[loot]>
    - flag player hephaestus.crate.pending_tier:<[tier]>
    - flag player hephaestus.crate.pending_tier_color:<[tier_color]>
    - flag player hephaestus.crate.animation_running:true

    # Narrate opening message
    - narrate "<&7>You insert the <&8>Hephaestus Key<&7>..."
    - playsound <player> sound:block_chest_open volume:1.0

    # Open GUI with gray border around rolling area
    ...
    - inventory open d:hephaestus_crate_gui
    ...
    # Define preview pool as actual ItemTags (mining themed)
    - define preview_pool <list>
    - define preview_pool <[preview_pool].include[<item[cobblestone]>]>
    - define preview_pool <[preview_pool].include[<item[coal]>]>
    ...
    - define preview_pool <[preview_pool].include[<item[copper_ingot]>]>
    ...
    - repeat 20:
        ...
        - if !<player.has_flag[hephaestus.crate.animation_running]>:
            - stop
    ...
    - define final_display_item <proc[build_hephaestus_loot_display_item].context[<[loot]>]>
    ...
    - flag player hephaestus.crate.animation_running:!
    ...
    - flag player hephaestus.crate.pending_loot:!
    - flag player hephaestus.crate.pending_tier:!
    - flag player hephaestus.crate.pending_tier_color:!

hephaestus_crate_gui:
    type: inventory
    inventory: chest
    gui: true
    title: <&8>Hephaestus Crate - Rolling...
    size: 27
...
roll_hephaestus_loot:
    ...
    - choose <[tier]>:
        - case MORTAL:
            - define pool <list>
            - define pool <[pool].include[cobblestone:16]>
            - define pool <[pool].include[coal:8]>
            - define pool <[pool].include[raw_iron:4]>
            - define pool <[pool].include[copper_ingot:4]>
            - define pool <[pool].include[emerald:8]>
            - define pool <[pool].include[bone_meal:16]>
            - define pool <[pool].include[flint:8]>
        ...
        - case HEROIC:
            ...
            - define pool <[pool].include[lapis_block:2]>
            - define choice <[pool].random>
            - define parts <[choice].split[<&co>]>
            - define material <[parts].get[1]>
            - define qty <[parts].get[2]>
        ...
        - case LEGENDARY:
            ...
            - define pool <[pool].include[hephaestus_key:2]>
            ...
            - if <[material]> == experience:
                - define loot_map <map[type=EXPERIENCE;amount=<[qty]>;display=+<[qty]> Experience]>
            - else if <[material]> == hephaestus_key:
                - define loot_map <map[type=CUSTOM;script=hephaestus_key;quantity=<[qty]>;display=Hephaestus Key x<[qty]>]>
            - else:
                - define display_name <[material].to_titlecase.replace[_].with[ ]>
                - define loot_map <map[type=ITEM;material=<[material]>;quantity=<[qty]>;display=<[display_name]> x<[qty]>]>
            - determine <[loot_map]>
        ...
        - case OLYMPIAN:
            - define loot_map <map[type=CUSTOM;script=vulcan_key;quantity=1;display=Vulcan Key x1]>
            - determine <[loot_map]>
...
build_hephaestus_loot_display_item:
    type: procedure
    debug: false
    definitions: loot
    script:
    ...
    - choose <[loot].get[type]>:
        ...
        - case TITLE:
            ...
            - determine <item[name_tag].with[display=<&7><&l><[loot].get[display]>;enchantments=mending,1;hides=ALL]>

# ============================================
# EARLY CLOSE HANDLER
# ============================================

hephaestus_crate_early_close:
    type: world
    debug: false
    events:
        on player closes hephaestus_crate_gui:
        ...
        - define loot <player.flag[hephaestus.crate.pending_loot]>
        - define tier <player.flag[hephaestus.crate.pending_tier]>
        - define tier_color <player.flag[hephaestus.crate.pending_tier_color]>
